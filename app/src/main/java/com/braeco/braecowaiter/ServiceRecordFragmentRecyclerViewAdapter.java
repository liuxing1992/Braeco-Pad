...
public class ServiceRecordFragmentRecyclerViewAdapter
        extends RecyclerView.Adapter<ServiceRecordFragmentRecyclerViewAdapter.ViewHolder> {

    private OnRefundListener onRefundListener;
    private Context context;
    private HashMap<Integer, Boolean> expand;

    public ServiceRecordFragmentRecyclerViewAdapter(OnRefundListener onRefundListener) {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if ("尚未到账".equals(BraecoWaiterApplication.todayRecords.get(position).get("serial"))) {
            holder.orderId.setTextSize(TypedValue.COMPLEX_UNIT_SP, 30);
            holder.orderId.setText("尚未到账");
        } else if ("未出票".equals(BraecoWaiterApplication.todayRecords.get(position).get("serial"))) {
            holder.orderId.setTextSize(TypedValue.COMPLEX_UNIT_SP, 30);
            holder.orderId.setText("未出票");
        } else if ("推送失败".equals(BraecoWaiterApplication.todayRecords.get(position).get("serial"))) {
            holder.orderId.setTextSize(TypedValue.COMPLEX_UNIT_SP, 30);
            holder.orderId.setText("推送失败");
        } else if ("推送中".equals(BraecoWaiterApplication.todayRecords.get(position).get("serial"))) {
            holder.orderId.setTextSize(TypedValue.COMPLEX_UNIT_SP, 30);
            holder.orderId.setText("推送中");
        } else {
            holder.orderId.setTextSize(TypedValue.COMPLEX_UNIT_SP, 40);
            String orderIdString = String.format("%04d",
                    Integer.parseInt((String)BraecoWaiterApplication.todayRecords.get(position).get("serial")));
            holder.orderId.setText(orderIdString);
        }

        if ("cash".equals(BraecoWaiterApplication.todayRecords.get(p).get("channel"))) {
            holder.payType.setText("现金");
            holder.payType.setBackgroundResource(R.drawable.shape_cash_pay);
        } else if ("prepayment".equals(BraecoWaiterApplication.todayRecords.get(p).get("channel"))) {
            holder.payType.setText("会员余额");
            holder.payType.setBackgroundResource(R.drawable.shape_cash_pay);
        } else if ("waiter".equals(BraecoWaiterApplication.todayRecords.get(p).get("channel"))) {
            holder.payType.setText("服务员下单");
            holder.payType.setBackgroundResource(R.drawable.shape_waiter_pay);
        } else if ("wx_pub".equals(BraecoWaiterApplication.todayRecords.get(p).get("channel"))) {
            holder.payType.setText("微信");
            holder.payType.setBackgroundResource(R.drawable.shape_online_pay);
        } else if ("p2p_wx_pub".equals(BraecoWaiterApplication.todayRecords.get(p).get("channel"))) {
            holder.payType.setText("微信P2P");
            holder.payType.setBackgroundResource(R.drawable.shape_online_pay);
        } else if ("alipay_qr_f2f".equals(BraecoWaiterApplication.todayRecords.get(p).get("channel"))) {
            holder.payType.setText("支付宝当面付");
            holder.payType.setBackgroundResource(R.drawable.shape_ali_pay);
        } else if ("alipay_wap".equals(BraecoWaiterApplication.todayRecords.get(p).get("channel"))) {
            holder.payType.setText("支付宝");
            holder.payType.setBackgroundResource(R.drawable.shape_ali_pay);
        } else if ("bfb_wap".equals(BraecoWaiterApplication.todayRecords.get(p).get("channel"))) {
            holder.payType.setText("百度钱包");
            holder.payType.setBackgroundResource(R.drawable.shape_online_pay);
        }

        long createDate = (Integer)BraecoWaiterApplication.todayRecords.get(p).get("create_date");
        Date createCalendar = new Date(createDate * 1000L);
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        sdf.setTimeZone(TimeZone.getDefault());
        holder.date.setText(sdf.format(createCalendar));

        if ("eatin".equals((String)BraecoWaiterApplication.todayRecords.get(p).get("type"))) {
            holder.tableId.setText("堂食" + (String)BraecoWaiterApplication.todayRecords.get(p).get("table") + "号桌");
        } else if ("membership".equals((String)BraecoWaiterApplication.todayRecords.get(p).get("type"))) {
            holder.tableId.setText("会员充值");
        } else if ("takeout".equals((String)BraecoWaiterApplication.todayRecords.get(p).get("type"))) {
            holder.tableId.setText("外带");
        }

        if (!BraecoWaiterUtils.notNull((String) BraecoWaiterApplication.todayRecords.get(p).get("phone")))
            holder.phone.setVisibility(View.INVISIBLE);
        holder.phone.setText("手机号：" + BraecoWaiterApplication.todayRecords.get(p).get("phone"));

        holder.list = (List<Map<String, Object>>)BraecoWaiterApplication.todayRecords.get(p).get("content");
        ...
        if ("发起退款".equals(BraecoWaiterApplication.todayRecords.get(p).get("refund"))) {
            holder.refund.setEnabled(true);
            holder.refund.setText((String)BraecoWaiterApplication.todayRecords.get(p).get("refund"));
            holder.refund.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    refund(context, String.valueOf(BraecoWaiterApplication.todayRecords.get(p).get("id")), p);
                }
            });
        } else {
    ...
    public int getItemCount() {
        return BraecoWaiterApplication.todayRecords.size();
    }
