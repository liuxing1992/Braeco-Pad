...
public class MeFragmentMenuSet extends BraecoAppCompatActivity
        implements
        View.OnClickListener,
        MeFragmentMenuSetAdapter.OnCheckListener,
    ...
    private MeFragmentMenuSetAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_me_fragment_menu_set);
        ...
        for (int i = 0; i < BraecoWaiterApplication.mSet.size(); i++) {
            isCheck.add(false);
        }
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                if (position == listView.getCount() - 2) {
                    ...
                    if (AuthorityManager.ableTo(Authority.MANAGER_DISH)) {
                        ...
                        AuthorityManager.showDialog(mContext, "新建套餐");
                    }
                } else if (position < listView.getCount() - 2) {
                    if (!isEditing) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.sale:
                ...
                else AuthorityManager.showDialog(mContext, "恢复/暂停售卖套餐");
                break;
            case R.id.betop:
                if (AuthorityManager.ableTo(Authority.MANAGER_DISH)) beTop();
                else AuthorityManager.showDialog(mContext, "置顶套餐");
                break;
            case R.id.delete:
                if (AuthorityManager.ableTo(Authority.MANAGER_DISH)) delete();
                else AuthorityManager.showDialog(mContext, "删除套餐");
                break;
            case R.id.change:
                if (AuthorityManager.ableTo(Authority.MANAGER_DISH)) move();
                else AuthorityManager.showDialog(mContext, "移动套餐");
                break;
        }
    }
    ...
    private void sell() {
        ...
        if (selects.isEmpty()) {
            // no one selected
            BraecoWaiterUtils.showToast(mContext, "请选择需要更改售卖状态的套餐");
        } else {
            boolean isAllSelling = true;
            for (int i = selects.size() - 1; i >= 0; i--) {
                if (!(Boolean) BraecoWaiterApplication.mSet.get(selects.get(i)).get("able")) {
                    isAllSelling = false;
                }
            }
            if (isAllSelling) {
                ...
                // pause selling
                String content = "确定将以下套餐暂停售卖吗？\n";
                if (selects.size() == 1) {
                    content = "确定将 " + BraecoWaiterApplication.mSet.get(selects.get(0)).get("name")
                            + " 暂停售卖吗？";
                } else {
                    for (int i = 0; i < selects.size(); i++) {
                        content += BraecoWaiterApplication.mSet.get(selects.get(i)).get("name") + "\n";
                    }
                }
                        ...
                        .onAny(new MaterialDialog.SingleButtonCallback() {
                            ...
                            public void onClick(@NonNull MaterialDialog materialDialog, @NonNull DialogAction dialogAction) {
                                if (dialogAction == DialogAction.POSITIVE) {
                                    ...
                                    for (int i = 0; i < selects.size(); i++) {
                                        data.put(BraecoWaiterApplication.mSet.get(selects.get(i)).get("id"));
                                    }
    ...
    private void move() {
        if (BraecoWaiterApplication.mButton.size() == 2) {
        ...
        if (selects.isEmpty()) {
            // no one selected
            BraecoWaiterUtils.showToast(mContext, "请选择需要移动的套餐");
        } else {
            ArrayList<String> strings = new ArrayList<>();
            for (int i = 1; i < BraecoWaiterApplication.mButton.size(); i++) {
                // add other categories to the dialog
            ...
            new MaterialDialog.Builder(this)
                    ...
                    .itemsCallbackSingleChoice(-1, new MaterialDialog.ListCallbackSingleChoice() {
                        ...
                        public boolean onSelection(MaterialDialog dialog, View view, final int which, CharSequence text) {
                            String content = "确定将以下套餐移动到 " + items[which] + " 吗？\n";
                            if (selects.size() == 1) {
                                content = "确定将 " + BraecoWaiterApplication.mSet.get(selects.get(0)).get("name")
                                        + " 移动到 " + items[which] + " 吗？\n";
                            } else {
    ...
    private void delete() {
        ...
        if (selects.isEmpty()) {
            // no one selected
            BraecoWaiterUtils.showToast(mContext, "请选择需要删除的套餐");
        } else {
    ...
    private void beTop() {
        ...
        if (selects.isEmpty()) {
            // no one selected
            BraecoWaiterUtils.showToast(mContext, "请选择需要置顶的套餐");
        } else {
            String content = "确定将以下套餐置顶吗？\n";
            if (selects.size() == 1) {
                content = "确定将 " + BraecoWaiterApplication.mSet.get(selects.get(0)).get("name")
                        + " 置顶吗？";
            } else {
    ...
    public class loading extends AsyncTask<String, Void, Boolean> {
        ...
        protected Boolean doInBackground(final String... args) {
            ...
            adapter = new MeFragmentMenuSetAdapter(mContext, MeFragmentMenuSet.this, isCheck);
